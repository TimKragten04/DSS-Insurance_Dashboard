...
class DataLoader:
    ...
    def __fetch_raw_data(self, base_url:str, target_field:str, save_file:str)->pd.DataFrame:
        ...
        self.__dataframe_to_csv(original_data,"../data/crime_rates",f"{save_file}")
    ...
    def __generate_crimerates_dataset(self,crimes:pd.DataFrame,population:pd.DataFrame, 
        ...
        data = pd.merge(data, region,left_on="region_code", right_on="Key").drop(columns="Key")
        self.__dataframe_to_csv(data, save_folder, file_name)
    ...
    def __initialize_dataset(self):
        ...
        with open("../data/selected_crimes_codes.json",'r') as file:
            code = json.load(file)
        crimecodes=[]
    ...
    def __get_region(self)->pd.DataFrame:
        ...
        """
        region = pd.read_csv(f"{self.data_path}/region_code.csv")
        return region
    ...
    def __check_and_update_local_csv(self):
        ...
        if both_updated :
            ...
            self.__dataframe_to_csv(log, save_folder="../data", file_name="update_log")
